...
## Integration Catalog Import

- Canonical import schema: [`docs/developer-integration-import.schema.json`](docs/developer-integration-import.schema.json).
- For every new integration, add a catalog import file at `clients/<integration_key>/catalog_import.json` that follows this schema.
- For Python integrations, the catalog `endpoint` normally uses `https://py-integrations.regos.uz/clients/<integration_key>` unless the integration is intentionally hosted elsewhere.
- Keep the catalog import file aligned with the integration implementation and README: same integration key, endpoint, handler, localized names/descriptions, docs URL, webhooks, and settings.
- The catalog import file should be ready to import through the developer cabinet. If a deployment-specific value is unknown, use the smallest valid placeholder and call it out in the final response.
