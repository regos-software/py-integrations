...
logger = setup_logger("meta_leadgen_crm_channel.ui")
...
def _load_template() -> str:
    try:
        ...
        logger.exception("Failed to load Meta Leadgen UI template: %s", error)
...
def _load_css() -> str:
    try:
        ...
        logger.exception("Failed to load Meta Leadgen UI CSS: %s", error)
...
def _load_i18n_bundle() -> Dict[str, Dict[str, str]]:
    ...
    for locale in _SUPPORTED_LOCALES:
        ...
        try:
            ...
            logger.exception("Failed to load Meta Leadgen UI i18n file %s: %s", path, error)
            bundle[locale] = {}
...
def _page_label(locale: str, page_name: Optional[str], *, connected: bool) -> str:
    name = str(page_name or "").strip()
    if name:
        return name
    return t(locale, "page_connected_fallback" if connected else "page_not_connected")


def _language_links(ctx: MetaLeadgenUiContext) -> str:
    links = []
    for locale in _SUPPORTED_LOCALES:
...
def _mode_data(ctx: MetaLeadgenUiContext) -> Dict[str, Any]:
    ...
    page_label = _page_label(locale, ctx.page_name, connected=connected)
    ...
    data: Dict[str, Any] = {
        ...
        "page": page_label,
        "notice_key": "",
        "steps": ("step_save_settings", "step_connect_page", "step_send_test_lead"),
        "actions": [],
    }
    ...
    if mode == "connected":
        data.update(
            ...
            steps=("step_connected_leads", "step_connected_fields", "step_connected_disconnect"),
            actions=[
...
def render_meta_leadgen_ui(ctx: MetaLeadgenUiContext) -> str:
    locale = normalize_locale(ctx.locale)
    data = _mode_data(ctx)
    ...
    return template.format(
        ...
        safe_page_label=_escape(str(data.get("page") or "")),
        safe_page_caption=_escape(t(locale, "page_caption")),
