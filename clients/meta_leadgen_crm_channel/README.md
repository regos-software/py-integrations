...
| `meta_client_field_mapping` | Нет | JSON Object | Нет | `Поля клиента` / `Mijoz maydonlari` / `Customer fields` | `Связь дополнительных ответов формы с доп. полями клиента CRM; имя, телефон и email заполняются автоматически` / `Formadagi qo'shimcha javoblarni CRM mijoz qo'shimcha maydonlariga bog'lash; ism, telefon va email avtomatik to'ldiriladi` / `Maps extra form answers to CRM customer custom fields; name, phone, and email are filled automatically` | `{}` / `{}` / `{}` |
| `meta_lead_field_mapping` | Нет | JSON Object | Нет | `Поля лида` / `Lid maydonlari` / `Lead fields` | `Связь дополнительных ответов формы с доп. полями лида CRM; все ответы формы уже сохраняются в описании лида` / `Formadagi qo'shimcha javoblarni CRM lid qo'shimcha maydonlariga bog'lash; barcha forma javoblari lid tavsifida saqlanadi` / `Maps extra form answers to CRM lead custom fields; all form answers are already saved in the lead description` | `{}` / `{}` / `{}` |
