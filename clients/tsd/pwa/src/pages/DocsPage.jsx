...
export default function DocsPage() {
  ...
  const statusLabel = useCallback((doc) => {
  ...
  const nothingLabel = useMemo(() => t('common.nothing') || 'Ничего не найдено', [t]);
  ...
  return (
    <section className="stack" id="docs">
      ...
      <div className="row">
        <span id="page-indicator" className="muted">{page} / {totalPages}</span>
        <div className="cluster">
          <button id="prev-page" type="button" className="btn small ghost" onClick={handlePrev} disabled={page <= 1}>
            {backLabel}
          </button>
          <button id="next-page" type="button" className="btn small ghost" onClick={handleNext} disabled={page >= totalPages}>
            {nextLabel}
          </button>
        </div>
      </div>
      ...
      <div id="docs-list" className="list" aria-live="polite">
        {loading && <div className="muted">{t('common.loading') || 'Загрузка...'}</div>}
        {!loading && error && <div className="muted">{String(error.message || error)}</div>}
        {!loading && !error && items.length === 0 && (
          <div className="muted">{nothingLabel}</div>
        )}
        {!loading && !error && items.map((doc) => (
          <button
            key={doc.id}
            type="button"
            className="item"
          ...
          >
            <div className="row">
              <div className="stack muted">
                ...
                <span className="muted truncate">{doc.partner?.name || ''}</span>
              </div>
