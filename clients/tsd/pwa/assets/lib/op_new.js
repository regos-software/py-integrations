...
async function startScan(ctx) {
  ...
  try {
    ...
    ensureOverlay(video); drawROI(video);

    if (!currentDeviceId) currentDeviceId = await pickBackCameraId();

    zxingControls = await reader.decodeFromVideoDevice(currentDeviceId ?? undefined, "preview", (result, err, controls) => {
      drawROI(video);
      if (result?.text) {
    ...
    // попробовать включить фонарик, если доступен
    try {
...
export async function screenOpNew(ctx, id) {
  ...
  if (btnScan)  btnScan.onclick  = () => startScan(ctx);
  if (btnClose) btnClose.onclick = () => stopScan(ctx);
