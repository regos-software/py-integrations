...
export async function screenDoc(ctx, id) {
  ...
  function renderOps(items) {
    ...
    const code = (item.code ?? "").toString().padStart(6, "0"); // 000001 формат

    const wrap = document.createElement("div");
    wrap.className = "item"; // компактность и адаптивность задаёт CSS для #ops-list

    // view mode: верхняя полоса (имя + иконки), под ней код/штрихкод, ещё ниже — qty/цены
    ...
    btnDel.onclick = async () => {
        ...
        if (ok && affected > 0) {
            wrap.remove();
            if (!ctx.$("ops-list").children.length) ctx.$("ops-list").innerHTML = `<div class="muted">${ctx.t("doc.no_ops") || ctx.t("common.nothing")}</div>`;
    ...
    btnSave.onclick = async () => {
        ...
        try {
        const payload = {
            items: [{ id: op.id, quantity: qty, cost, ...(Number.isFinite(price) ? { price } : {}) }]
        };
        ...
        if (ok && affected > 0) {
            ...
            op.quantity = qty;
            op.cost = cost;
            op.price = Number.isFinite(price) ? price : undefined;

            // точечно обновляем нижнюю строку
