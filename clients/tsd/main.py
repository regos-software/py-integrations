...
class TsdIntegration(ClientBase):
    ...
    @staticmethod
    def _normalize_external_api_result(raw_action: str, data: Any) -> Any:
        if str(raw_action).lower() not in {"batch.run", "batch.batch", "batch"}:
            return data
        ...
        # The TSD PWA external contract is the raw REGOS batch shape:
        # {"ok": true, "result": [steps]}. The typed API client keeps batch
        # steps under result.responses, so unwrap it only at this boundary.
        normalized = dict(data)
        normalized["result"] = responses
        return normalized
    ...
    async def handle_external(self, data: dict) -> Any:
        try:
            ...
            json_result = self._normalize_external_api_result(
                raw_action,
