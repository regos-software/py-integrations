...
Служебные Redis-ключи используют короткий namespace `bc`: основная очередь `bc:s:e`, DLQ `bc:s:dlq`, dedupe очереди `bc:qd:<connected_integration_id>:<hash>`, dedupe синхронизации `bc:sy:<connected_integration_id>:<ticket_id>:<client_id>:<hash>`.
