...
- Переоткрытие закрытого обращения при новом сообщении клиента.
...
| `ticket_subject_template` | Нет | String | `Шаблон темы обращения` / `Murojaat mavzusi shabloni` / `Case subject template` | `Шаблон заголовка обращения. Поддерживаются переменные {visitor_id}, {display_name}, {current_date}` / `Murojaat sarlavhasi shabloni. {visitor_id}, {display_name}, {current_date} o'zgaruvchilari qo'llab-quvvatlanadi` / `Template for case subject. Supports {visitor_id}, {display_name}, {current_date}` | `Web chat {visitor_id}` / `Web chat {visitor_id}` / `Web chat {visitor_id}` |
...
| `chat_theme_vars` | Нет | String (JSON) | `Тема чата (CSS переменные)` / `Chat mavzusi (CSS o'zgaruvchilari)` / `Chat theme (CSS variables)` | `JSON-объект с цветовыми переменными интерфейса. Поддерживаются ключи: bg, card, line, text, muted, accent, mine, other, system` / `Interfeys rang o'zgaruvchilari uchun JSON obyekt. Qo'llab-quvvatlanadigan kalitlar: bg, card, line, text, muted, accent, mine, other, system` / `JSON object with UI color variables. Supported keys: bg, card, line, text, muted, accent, mine, other, system` | `{"accent":"#0f766e","bg":"#f2fbf9"}` / `{"accent":"#0f766e","bg":"#f2fbf9"}` / `{"accent":"#0f766e","bg":"#f2fbf9"}` |
...
4. При необходимости настройте внешний вид через `chat_theme_vars`.
