...
- `notification_count` — получение текущего количества уведомлений о новых сообщениях оператора.
...
Счетчик хранится отдельно для webchat и увеличивается только по новым видимым сообщениям не от `Client`; CRM-счетчик непрочитанных не используется.
