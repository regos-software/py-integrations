...
logger = setup_logger("uzumtezkor")


class UzumTezkorError(Exception):
...
def _error_list(code: int, description: str) -> List[Dict[str, Any]]:
    return [_error(code, description)]
...
def _path(envelope: Dict[str, Any]) -> str:
    ...
    if normalized.lower().startswith("uzumtezkor/"):
        normalized = normalized[len("uzumtezkor/") :]
...
class UzumTezkorIntegration(ClientBase):
    integration_key = "marketplace_uzum_tezkor"
    redis_prefix = "mp:uz"
    ...
    async def handle_external(self, envelope: Optional[Dict[str, Any]] = None, **kwargs: Any) -> Any:
        ...
        try:
            ...
            if not valid:
                if path.startswith("v1/order") or path.startswith("order"):
                    return _json({"reason": auth_error["description"]}, 401)
                return _json(_error_list(auth_error["code"], auth_error["description"]), 401 if auth_error["code"] == 401 else 500)
            ...
            if order_id is not None:
                ...
                if method == "DELETE" and subpath == "":
                    return _json(await self._cancel_order(order_id))
                if method == "PUT" and subpath == "":
                    return _json(_error_list(422, "Заказ не может быть обновлен"), 422)

            return _json(_error_list(404, "Route not found"), 404)
        except UzumTezkorError as error:
            if error.status_code == 404:
                return _json(_error_list(404, error.description), 404)
            return _json(_error_list(error.code, error.description), error.status_code)
        except httpx.HTTPStatusError as error:
            logger.warning("Uzum REGOS HTTP error: %s", error)
            return _json(_error_list(500, "Внутренняя ошибка сервера"), 500)
        except Exception as error:
            logger.exception("Uzum external handling failed: %s", error)
            return _json(_error_list(500, "Внутренняя ошибка сервера"), 500)
    ...
    async def _load_integration(self) -> str:
        ...
        if isinstance(cached, dict):
            if not bool(cached.get("active")):
                raise UzumTezkorError(113422, "Integration is inactive")
        ...
        if row is None:
            raise UzumTezkorError(113422, "Интеграция не найдена")
        if not bool(getattr(row, "is_active", False)):
            ...
            raise UzumTezkorError(113422, "Интеграция не активна")
    ...
    async def _load_settings(self) -> Dict[str, str]:
        ...
        if not lock_token:
            raise UzumTezkorError(113423, "Settings cache lock timeout", status_code=500)
        try:
            cached = await redis_get_json(self._settings_cache_key())
            if isinstance(cached, dict):
                self._settings = {str(key): str(value or "") for key, value in cached.items() if str(key)}
    ...
    async def _item_ext_all(self, stock_id: int, price_type_id: int, *, zero_price: bool) -> List[Dict[str, Any]]:
        ...
        async with RegosAPI(self._ci()) as api:
            while True:
                ...
                if not response.ok:
                    raise UzumTezkorError(111321, "REGOS item ext request rejected", status_code=500)
                chunk = [
    ...
    async def _composition(self, store_id: int) -> Dict[str, Any]:
        if store_id <= 0:
            raise UzumTezkorError(400, "Неверный формат storeId")
    ...
    def _map_catalog_item(self, ext: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        icps = _text(item.get("icps")).strip()
        if price <= 0 or not barcode or not icps:
            return None
        is_pcs = _is_pcs(_nested(item, "unit.type"))
        image_url = _text(ext.get("image_url")).strip()
        images = [{"url": image_url, "hash": _sha1(image_url)}] if image_url else []
        return {
            ...
            "serviceCodesUz": {"mxikCodeUz": icps},
    ...
    async def _availability(self, store_id: int) -> Dict[str, Any]:
        ...
        for ext in items:
            ...
            if item_id <= 0:
                continue
            qty = _to_decimal(_nested(ext, "quantity.allowed"))
            if qty < 0:
                qty = Decimal("0")
            result.append({"id": str(item_id), "stock": qty})
        return {"items": result}

    async def _create_order(self, order: Dict[str, Any]) -> Dict[str, Any]:
        eats_id = _text(order.get("eatsId") or order.get("EatsId")).strip()
        if not eats_id:
            raise UzumTezkorError(400, "Invalid order payload")
        ...
        if not lock_token:
            raise UzumTezkorError(409, "Order is already being processed", status_code=409)
        try:
            cached_order_id = await self._read_order_dedupe(eats_id)
            if cached_order_id > 0:
                return {"result": "OK", "orderId": str(cached_order_id)}
    ...
    async def _create_order_unlocked(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(delivery, dict):
            ...
            document["delivery_date"] = _parse_datetime_to_unix(
                delivery.get("courierArrivementDate") or delivery.get("deliveryDate") or delivery.get("DeliveryDate")
            )
        ...
        for item_id in item_ids:
            if item_id not in found:
                raise UzumTezkorError(113422, f"Товар с ID {item_id} не найден в списке номенклатуры", status_code=500)
        ...
        if not response.ok:
            raise UzumTezkorError(113422, "Order creation rejected", status_code=500)
        doc_id = _to_int(getattr(response.result, "new_id", None))
        if doc_id <= 0:
            raise UzumTezkorError(113422, "Заказ не создан", status_code=500)
    ...
    async def _item_groups(self) -> List[Dict[str, Any]]:
        ...
        if not response.ok:
            raise UzumTezkorError(111321, "REGOS item groups request rejected", status_code=500)
        return [
            row.model_dump(mode="json", by_alias=True)
            for row in (response.result or [])
        ]
    ...
    async def _item_short_all(self, item_ids: List[int]) -> List[Dict[str, Any]]:
        ...
        async with RegosAPI(self._ci()) as api:
            while True:
                ...
                if not response.ok:
                    raise UzumTezkorError(111321, "REGOS item short request rejected", status_code=500)
                chunk = [
                    row.model_dump(mode="json", by_alias=True)
                    for row in (response.result or [])
                ]
    ...
    async def _order_doc(self, *, order_id: Optional[int] = None, external_code: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        if not response.ok:
            raise UzumTezkorError(111321, "REGOS order request rejected", status_code=500)
        rows = [
            row.model_dump(mode="json", by_alias=True)
            for row in (response.result or [])
        ]
    ...
    async def _order_operations(self, order_id: int) -> List[Dict[str, Any]]:
        ...
        if not response.ok:
            raise UzumTezkorError(111321, "REGOS order operations request rejected", status_code=500)
        return [
            row.model_dump(mode="json", by_alias=True)
            for row in (response.result or [])
        ]
    ...
    async def _get_order(self, order_id: int) -> Dict[str, Any]:
        ...
        if not order:
            raise UzumTezkorError(404, "Заказ не найден", status_code=404)
    ...
    async def _cancel_order(self, order_id: int) -> Dict[str, Any]:
        ...
        if _order_status_id(order) != 27:
            ...
            if not response.ok:
                raise UzumTezkorError(111321, "REGOS order status update rejected", status_code=500)
        return {"result": "OK"}
